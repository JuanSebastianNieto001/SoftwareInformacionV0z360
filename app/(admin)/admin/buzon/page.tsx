...
export default async function PaginaBuzonAdmin() {
  ...
  const abiertos = (casos ?? []).filter(
    (c) => c.estado !== "cerrada" && c.estado !== "rechazada",
  ).length;
  ...
  return (
    <>
      <EncabezadoPagina
        titulo="Buzón"
        descripcion="Sugerencias, quejas y no conformidades. Cerrar un caso exige registrar la causa y la acción tomada; descartarlo exige justificarlo."
        acciones={
          <Button variant="outline" asChild>
            <Link href="/api/admin/buzon/csv" prefetch={false}>
      ...
      {!casos || casos.length === 0 ? (
        ...
        <>
          <p className="mb-3 text-sm text-muted-foreground">
            {casos.length} registro{casos.length === 1 ? "" : "s"} · {abiertos} sin cerrar
          </p>
